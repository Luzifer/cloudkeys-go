<template>
  <div>
    <b-row align-h="center">
      ...
      <b-col
        cols="12"
        v-if="this.$store.state.account_info.data_raw.length == 0 && !this.$store.state.account_info.master_password">
        <b-alert
          show
          variant="primary">
      ...
      <b-col
        cols="8"
        v-if="this.$store.state.account_info.loaded && !this.$store.state.account_info.master_password">
        <b-form
          @submit.prevent="onMasterPassword"
          id="masterPassword">
      ...
      <b-col
        cols="12"
        v-if="!this.$store.state.cryptocore_available">
        <b-alert
          show
          variant="warning">
      ...
      <b-col
        cols="12"
        v-if="this.$store.state.account_info.data.length == 0 && this.$store.state.account_info.master_password">
        <b-alert
          show
          variant="primary">
    ...
    <b-row
      v-if="this.$store.state.account_info.data.length > 0"
      class="w-100">
      <b-col cols="4">
        <b-form>
          <b-form-group>
        ...
        <b-list-group class="mt-3">
          <b-list-group-item
            v-for="entry in $store.getters.filtered_entries"
            button
            :key="entry.id"
...
export default {
  ...
  methods: {
    ...
    copyClass(component) {
      if (this.copySuccess === component) {
        return 'success'
      }
      if (this.copyError === component) {
        return 'danger'
      }
      return 'primary'
    },
    ...
    onMasterPassword(e) {
      ...
      const form = document.getElementById('masterPassword')
      const entry = {}
